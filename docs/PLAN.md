...
### 2.10 C2-1 方向启动（2026-06-13）

DE-3 / DE-1 / AR-1 / AR-2 完成后, 已在 4 选 1 中选定
**C2-1 trajectory-level modeling** 作为下一阶段主线方向。

**C2-1 核心问题**:
不再显式建模"信号灯状态 / 排队状态 / 相位时长"等
state-level 信息, 把精力放到"如何让 trajectory LSTM 本身学到更精确的
演化"上。如果连 trajectory capacity 都不够, 谈 state injection 是
"在一条已经触顶的管道上换阀门"。

**C2-1 第一变体 (``C2-1-MV1``) 设计**:
- 在 ``TrajectoryGenerator`` 父类的 trajectory encoder 上叠加
  第 2 层 ``nn.LSTMCell`` (``traj_lstm_layer2``), 保持 hidden_size
  不变, 隔离"深度 vs 宽度"两个变量。
- 完全不动 state 分支、decoder、light embedding、loss — 这是一个
  干净的 isolated test, 验证 trajectory 容量是否是 1.6× 差距的瓶颈。
- 与 DE-3 / DE-1 / AR-1 / AR-2 等 state injection 模式**正交**, 可
  单独启用, 也可与 ``--minimal_viable_mode`` 组合验证
  "C2-1 + state hidden init 拼接"联合效果。
- 预计在 ``test + 20`` 口径下得到一个二元信号:
  - **C2-1-MV1 接近 DE-3** (24.6 附近) → trajectory 容量不是瓶颈,
    "显式交通状态记忆"才是该走的方向; 但 AR-1/AR-2 已否定这条路线,
    整个方向宣告失败, 转入 Phase E"承认负结果"。
  - **C2-1-MV1 明显好于 DE-3** (例如 20 以内) → trajectory 容量是
    真正的瓶颈, 可以继续在 trajectory 方向 (C2-1) 继续做深度 /
    宽度 / attention / 多步未来损失等细化变体。
  - **C2-1-MV1 比 DE-3 差** → "加深 encoder"本身有副作用, 应回到
    原始 1 层 + 其它 trajectory-level 改动 (位置编码、多步损失等)。

**启动状态 (2026-06-13)**:
- ✅ 模型实现落地 (``D2TP/models.py`` 父类 + 子类 forward 都加 layer2)
- ✅ CLI 开关 ``--c2_1_trajectory_level_mode`` (train.py + evaluate_model.py)
- ✅ Smoke test 通过 (``D2TP/c2_1_smoke_test.py``, 7 个测试全过)
- ✅ 训练脚本就绪 (``scripts/run_c2_1.sh``)
- ✅ 232 个 protocol tests 全部通过, 无回归
- ⏳ 实际训练 + 评估待执行
