...
- 但在旧 best 上做最小增量 refine 后，`val/test + 20` 都有实质下降，说明
  `macro rollout` 方向成立，只是当前强度还不足以追平 baseline。
- 更保守的 low-lr continuation 已经判负；当前最值得推进的不是再改结构，
  也不是继续扫 continuation 超参，而是把增强过的 decoder state path 带回训练阶段重新适配。
...
low-lr continuation 已判负：`20-batch val` 快照
`77.511 / 154.035 -> 69.903 / 138.006 -> 83.454 / 165.355`，没有接近
`Stage 47` 的 best。因此当前 Phase B 的前置结论是：先强化 state-to-decoder
耦合，再决定是否需要新一轮短训。目标优先仍是定位问题，而不是写结果故事。
...
默认先跑**离线 eval**，且优先基于 `..._cycle_rollout_refine1` 这类已经适配过
macro rollout 的 checkpoint：
...
在继续任何新训练前，先验证 state path 的数值耦合是否过弱。

当前已知证据：

1. 四个单变量消融在 `val + 20` 上几乎不动
2. base 与 `all_off` 在 `val + num_samples=1` 上也几乎重合
3. decoder step residual 平均范数约 `0.0278`
4. queue hidden 平均范数约 `2.1306`
5. `step_residual / queue_hidden ≈ 1.3%`
6. 仅在推理时把 `decoder_state_residual_scale` 提高到 `4.0` 时：
   - `val + 1`: `59.349 / 121.138 -> 59.205 / 120.487`
   - `val + 20`: `51.607 / 103.955 -> 51.383 / 103.133`
   - `test + 20`: `34.911 / 69.133 -> 35.147 / 69.575`

因此默认下一步应是：

1. 保留新的 `decoder_state_residual_scale` 旋钮
2. 直接在 `decoder_state_residual_scale > 1.0` 的协议上做最小短程 refine
3. 先看 `val + 20` 是否继续保持正向
4. 只有 `val + 20` 仍为正向时，再补 `test + 20`
